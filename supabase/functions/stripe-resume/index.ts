// supabase/functions/stripe-resume/index.ts
//
// Desfaz cancel_at_period_end. Exige OWNER.
import { adminClient, orgRoleOf } from "../_shared/db.ts";
import { stripeClient } from "../_shared/stripe.ts";
import { resumeSubscription } from "../_shared/stripe-billing.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const out = await resumeSubscription(sb, stripe, { orgId });
    return json(req, { data: out });
  } catch (err) {
    const status = (err as any)?.status ?? 500;
    const message = (err as Error)?.message ?? "erro-interno";
    if (status >= 500) console.error("stripe-resume erro", message);
