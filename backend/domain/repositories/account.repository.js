...
export const AccountRepository = {
  ...
  async findById(id, userId) {
    return prisma.account.findFirst({ where: { id, userId, active: true } });
  },
  ...
  async findOwnerById(id) {
    return prisma.account.findUnique({ where: { id }, select: { userId: true } });
  },
  ...
  async upsertSync(userId, id, data) {
    ...
    if (owned) {
      return prisma.account.update({ where: { id }, data: { ...data, active: data.active !== false } });
    }
    const foreign = await prisma.account.findUnique({ where: { id }, select: { userId: true } });
    if (foreign) {
  ...
  async createInactiveTombstone(userId, id, clientUpdatedAt) {
    ...
    const owned = await prisma.account.findFirst({ where: { id, userId } });
    if (owned) {
      return prisma.account.update({ where: { id }, data: { active: false, updatedAt: when } });
    }
    const foreign = await prisma.account.findUnique({ where: { id }, select: { userId: true } });
    if (foreign) {
      const err = new Error('id-em-uso');
