...
router.get('/:id', requirePermission('budgets:read'), asyncHandler(async (req, res) => {
  const budget = await BudgetService.getById(req.params.id, req.user.id);
  res.json({ data: budget });
}));

// PATCH /api/v1/budgets/:id
router.patch('/:id', requirePermission('budgets:write'), validateBody(budgetSchema.partial()), asyncHandler(async (req, res) => {
  const budget = await BudgetService.update(req.params.id, req.user.id, req.body);
  res.json({ data: budget });
}));

// DELETE /api/v1/budgets/:id — soft delete
router.delete('/:id', requirePermission('budgets:delete'), asyncHandler(async (req, res) => {
