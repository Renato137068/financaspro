...
// GET /api/v1/accounts
router.get('/', requirePermission('accounts:read'), asyncHandler(async (req, res) => {
  const result = await AccountService.list(req.user.id);
  res.json(result);
}));

// POST /api/v1/accounts
router.post('/', requirePermission('accounts:write'), validateBody(accountSchema), asyncHandler(async (req, res) => {
