...
import { validateBody, recurringSchema, validateParams, idParamSchema } from '../middleware/validate.js';
...
router.get('/:id', validateParams(idParamSchema), requirePermission('recurring:read'), asyncHandler(async (req, res) => {
  const item = await RecurringService.getById(req.params.id, req.user.id);
  res.json({ data: item });
}));

// PATCH /api/v1/recorrentes/:id
router.patch('/:id', validateParams(idParamSchema), requirePermission('recurring:write'), validateBody(recurringSchema.partial()), asyncHandler(async (req, res) => {
  const item = await RecurringService.update(req.params.id, req.user.id, req.body);
  res.json({ data: item });
}));

// DELETE /api/v1/recorrentes/:id — soft delete
router.delete('/:id', validateParams(idParamSchema), requirePermission('recurring:delete'), asyncHandler(async (req, res) => {
