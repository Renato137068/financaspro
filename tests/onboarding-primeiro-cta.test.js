...
describe('primeiro CTA em instalação limpa', function() {
  test('iniciar() não agenda convite — onboarding via card do Resumo', function() {
    ...
    expect(document.getElementById('onboarding-invite')).toBeNull();
  });

  test('clicar Registrar Transação marca interação sem convite prévio', function() {
    global.ONBOARDING.iniciar();
  ...
  test('tour só abre com abrirTourExplicito', function() {
    global.ONBOARDING.iniciar();
    jest.advanceTimersByTime(3000);
    expect(global.ONBOARDING._estado().convite).toBe(false);
